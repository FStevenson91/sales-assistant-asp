...
*   **Environment Setup** (in `.env` file):
    ...
    GOOGLE_CLOUD_LOCATION=Enter desired GCP region (Gemini uses global endpoint by default) (us-central1):
    ```
...
*   `--region`: GCP region (default: `Enter desired GCP region (Gemini uses global endpoint by default) (us-central1):`).
...
uvx agent-starter-pack create my-automated-agent \
  -a adk_base \
  -d cloud_run \
  --region Enter desired GCP region (Gemini uses global endpoint by default) (us-central1): \
  --auto-approve
```
